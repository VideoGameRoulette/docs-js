...
const Header = ({ title, version, state, callback }) => {
    return (
        <header className="w-full h-full bg-gray-800 text-white flex justify-between items-center gap-4 py-4 md:px-6 px-2">
            <div className="h-full flex justify-start items-center gap-4">
                <div className="text-lg font-bold">{title}</div>
                <div className="bg-gray-600 text-gray-200 rounded-md px-2 py-1 font-semibold md:text-base text-sm">v{version}</div>
            </div>
            <div className="flex justify-end items-center">
                ...
                <MenuButton className="block md:hidden" name="menu">
